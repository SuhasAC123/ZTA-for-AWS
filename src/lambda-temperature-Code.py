...
S3_BUCKET_NAME = os.environ.get('S3_BUCKET_NAME', 'temperatures3')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        response = table.query(
            KeyConditionExpression=boto3.dynamodb.conditions.Key('SensorID').eq('temperature')
        )
        items = response.get('Items', [])
        if not items:
            print("No items found for the specified SensorID.")
            return {
                'statusCode': 200,
                'body': json.dumps('No data found for SensorID: temperature')
            }
    ...
    try:
        ...
        error_message = e.response['Error']['Message']
        print(f"Error uploading data to S3: {error_message}")
        return {
            'statusCode': 500,
            'body': json.dumps(f'Failed to upload data to S3: {error_message}')
        }

    return {
        'statusCode': 200,
        'body': json.dumps('Successfully transferred temperature data to S3')
    }
